...
final class DetectionResultView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if tracks.isEmpty {
            for detection in detections {
                drawDetection(detection, in: context)
            }
        } else {
            for track in tracks {
                drawTrack(track, in: context)
            }
        }
    ...
    private func drawDetection(_ detection: Detection, in context: CGContext) {
        let drawRect = convertedRect(from: detection.rect)
        let color = colorForClass(id: detection.classID)
        drawBox(drawRect, color: color, context: context)
        drawLabel(
    ...
    private func drawTrack(_ track: TrackResult, in context: CGContext) {
        let drawRect = convertedRect(from: track.detection.rect)
        let color = colorForTrack(id: track.trackID)
        let suffix = track.isPredictionOnly ? " pred" : ""
        drawBox(drawRect, color: color, context: context)
        drawLabel(
            "#\(track.trackID) \(track.detection.className) \(Int(track.detection.confidence * 100))%\(suffix)",
            at: drawRect,
            color: color
        )
    }

    private func drawBox(_ rect: CGRect, color: UIColor, context: CGContext) {
        context.saveGState()
    ...
    private func colorForTrack(id: Int) -> UIColor {
        let hue = CGFloat((abs(id) * 37) % 360) / 360.0
        return UIColor(hue: hue, saturation: 0.95, brightness: 0.95, alpha: 1.0)
    }
}
