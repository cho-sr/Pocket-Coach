...
final class LiveTrackingViewController: UIViewController {
    ...
    private let stateQueue = DispatchQueue(label: "app.yolo_bytetrack.live.state")
    private let inferenceQueue = DispatchQueue(label: "app.yolo_bytetrack.live.inference", qos: .userInitiated)
    private let tracker = ByteTrack(configuration: .sportsTracking)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        title = DetectionMode.track.title
    ...
    private func setupPipeline() {
        do {
            pipeline = try DetectorPipeline(configuration: .tracking1024x576)
            statusLabel.text = "Track: ready"
        } catch {
            ...
            do {
                ...
                self.stateQueue.async {
                    ...
                    DispatchQueue.main.async {
                        self.statusLabel.text = self.statusText(result: result, tracks: tracks, fps: fps)
                        self.resultView.update(image: nil, detections: result.detections, tracks: tracks)
                    }
                ...
                self.stateQueue.async {
                    ...
                    DispatchQueue.main.async {
                        self.statusLabel.text = "Live tracking failed: \(error)"
                    }
    ...
    private func statusText(result: DetectionRunResult, tracks: [TrackResult], fps: Double) -> String {
        let input = "\(result.inputShape[3])x\(result.inputShape[2])"
        return "Track: \(tracks.count) tracks | \(result.detections.count) detections | \(String(format: "%.1f", fps)) FPS | \(String(format: "%.1f", result.inferenceTimeMs)) ms | input \(input)"
    }
}

extension LiveTrackingViewController: CameraServiceDelegate {
