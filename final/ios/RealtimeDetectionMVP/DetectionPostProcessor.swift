...
final class DetectionPostProcessor {
    ...
    func parse(rawOutput: [Float], inputWidth: CGFloat = 1.0, inputHeight: CGFloat = 1.0) -> [Detection] {
        ...
        for start in stride(from: 0, to: rawOutput.count, by: 6) {
            ...
            let usesAbsoluteInputPixels = Swift.max(
                Swift.max(rawX1, rawY1),
                Swift.max(rawX2, rawY2)
            ) > 1.0
    ...
    private func normalizedRect(
        ...
        let normalizedCenterX = usesAbsoluteInputPixels ? (centerX / inputWidth) : centerX
        let normalizedCenterY = usesAbsoluteInputPixels ? (centerY / inputHeight) : centerY
        let normalizedWidth = usesAbsoluteInputPixels ? (width / inputWidth) : width
        let normalizedHeight = usesAbsoluteInputPixels ? (height / inputHeight) : height

        let x1 = (normalizedCenterX - (normalizedWidth * 0.5)).clamped(to: 0.0...1.0)
        let y1 = (normalizedCenterY - (normalizedHeight * 0.5)).clamped(to: 0.0...1.0)
        let x2 = (normalizedCenterX + (normalizedWidth * 0.5)).clamped(to: 0.0...1.0)
        let y2 = (normalizedCenterY + (normalizedHeight * 0.5)).clamped(to: 0.0...1.0)
